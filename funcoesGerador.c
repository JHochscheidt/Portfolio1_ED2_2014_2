...
char *geradorDeSeparador(unsigned int quant, char *texto){
	...
	while(quant > 0){
		printf("C");
		teste = rand() % 4;
		if(teste == 0) 		c = (char) (nChar32 + (rand() % 16)) ; // separadores de 32 a 47
		else if(teste == 1)	c = (char) (nChar58 + (rand() % 7)); // separadores de 58 a 64
		else if(teste == 2) c = (char) (nChar91 + (rand() % 6)); // separadores de 91 a 96
